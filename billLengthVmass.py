...
sns.scatterplot(data=df, x='Culmen Length (mm)', y='Body Mass (g)', hue='Species', style='Sex', 
                palette=palette, s=80, edgecolor='k', linewidth=0.5, alpha=0.85, markers=['o', '^'])
plt.title('Bill Length vs Body Mass by Species and Sex', fontsize=14)
...
plt.tight_layout()
plt.savefig('final_fig3.pdf', format='pdf', bbox_inches='tight')
plt.close()